...
def home_embed():
    ...
    em.description = (
        ...
        f"**Prefix actuel :** `{p}`\n\n"
        f"> 🔨 **Blacklist** — Commandes de blacklist\n"
        f"> ⛔ **Super Blacklist** — Blacklist renforcée\n"
...
def bl_embed():
    p = get_config("prefix") or DEFAULT_PREFIX
    em = discord.Embed(title="🔨  Blacklist", color=embed_color())
    em.description = (
        f"```\n"
        f"{p}bl @user [raison]  → Blacklist un utilisateur\n"
        f"{p}unbl @user         → Retirer la blacklist\n"
        f"{p}bl                 → Afficher la blacklist\n"
        f"{p}blinfo @user       → Infos sur un utilisateur\n"
        f"```\n"
        f"ℹ️ Tu peux seulement blacklist les personnes avec un rang **inférieur** au tien."
    )
    em.set_footer(text="Made by gp ・ Bot BL")
    return em
...
def superbl_embed():
    p = get_config("prefix") or DEFAULT_PREFIX
    em = discord.Embed(title="⛔  Super Blacklist", color=embed_color())
    em.description = (
        f"```\n"
        f"{p}superbl @user [raison]  → Super blacklist\n"
        f"{p}unsuperbl @user         → Retirer super bl\n"
        f"{p}superbl                 → Afficher la super bl\n"
        f"```\n"
        f"ℹ️ Un super bl ne peut **pas** être retiré avec `unbl`.\n"
        f"Seul un **Sys+** peut utiliser `unsuperbl`."
    )
    em.set_footer(text="Made by gp ・ Bot BL")
    return em
...
def hierarchy_embed():
    ...
    em.description = (
        ...
        "> 🔧 **Sys** — Ajouter/retirer owner & wl, bl tout le monde\n"
        "> ⭐ **Owner** — Ajouter/retirer wl, bl ceux en dessous\n"
        "> ✅ **Wl** — Bl ceux sans rang uniquement\n\n"
        "ℹ️ Un rang ne peut **jamais** bl quelqu'un de rang égal ou supérieur."
    )
    em.set_footer(text="Made by gp ・ Bot BL")
    return em
...
class HelpDropdown(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="Blacklist", emoji="🔨", value="bl"),
            discord.SelectOption(label="Super Blacklist", emoji="⛔", value="superbl"),
    ...
    async def callback(self, interaction: discord.Interaction):
        embeds = {
            "home": home_embed, "bl": bl_embed, "superbl": superbl_embed,
            "perms": perms_embed, "system": system_embed, "hierarchy": hierarchy_embed,
        }
